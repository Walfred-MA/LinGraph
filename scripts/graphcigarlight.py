...
def path_alignment_size(cigar):
        """Return graph-path bases aligned by M, =, or X operations."""
        return sum(
                int(length)
...
def prepend_query_insertion(cigar, size):
        """Put query-only bases at a path's aligned-query beginning."""
        if size <= 0:
                return cigar
        leading_hard_clip = re.match(r'^\d+H', cigar)
        if leading_hard_clip is None:
                return f"{size}I{cigar}"
        split = leading_hard_clip.end()
        return f"{cigar[:split]}{size}I{cigar[split:]}"


def append_query_insertion(cigar, size):
        """Put query-only bases at a path's aligned-query end."""
        if size <= 0:
                return cigar
        trailing_hard_clip = re.search(r'\d+H$', cigar)
        if trailing_hard_clip is None:
                return f"{cigar}{size}I"
        split = trailing_hard_clip.start()
        return f"{cigar[:split]}{size}I{cigar[split:]}"


def filter_short_path_alignments(pathes, cigars, gregions, qregions,
                                 minimum_size=MIN_PATH_ALIGNMENT_SIZE):
        """Drop short paths and transfer their query spans as insertions."""
        counts = {len(pathes), len(cigars), len(gregions), len(qregions)}
        if len(counts) != 1:
                raise ValueError(
                        "graph path/CIGAR/reference/query component counts differ: "
                        f"{len(pathes)}/{len(cigars)}/{len(gregions)}/"
                        f"{len(qregions)}"
                )
        keep = [
                path_alignment_size(cigar) >= minimum_size
                for cigar in cigars
        ]
        if not any(keep):
                return [], [], [], []
        ...
        kept = []
        leading_query_start = None
        for retain, path, cigar, gregion, qregion in zip(
                keep, pathes, cigars, gregions, qregions,
        ):
                query_start, query_end = map(int, qregion)
                if retain:
                        if not kept and leading_query_start is not None:
                                inserted = max(0, query_start - leading_query_start)
                                cigar = prepend_query_insertion(cigar, inserted)
                ...
                if kept:
                        previous = kept[-1]
                        previous_start, previous_end = previous[3]
                        extended_end = max(previous_end, query_end)
                        previous[1] = append_query_insertion(
                                previous[1], extended_end - previous_end,
                        )
                        previous[3] = (previous_start, extended_end)
                elif leading_query_start is None:
                        leading_query_start = query_start
                else:
                        leading_query_start = min(
                                leading_query_start, query_start,
                        )

        return tuple([item[index] for item in kept] for index in range(4))


def no_path_graph_cigar(queryname, queryseq, ifaddinsert):
        """Represent a wholly unaligned query when insertion payloads are wanted."""
        if ifaddinsert:
                return (
                        queryname, "", queryseq, "",
                        f"0_{len(queryseq)}",
...
def alignment_polish(name, path, cigar, gregion, qregion):
        ...
        cigarstrings = ["".join(cigarstr) for cigarstr in cigars]
        pathes, cigarstrings, gregions, qregions = filter_short_path_alignments(
                pathes, cigarstrings, gregions, qregions,
        )

        newcigars= "".join(["{}:{}".format(thename,cigarstr) if thename[0] == '>' else "{}:{}".format(thename,cigarstr) for thename,cigarstr,rposi,qposi in zip(pathes,cigarstrings,gregions, qregions)])
...
def graphcigar(graphfile, queryfile, alignfile, ifaddinsert = True):
        ...
        if len(allaligns) == 0:
                return no_path_graph_cigar(queryname, queryseq, ifaddinsert)
...
def main(args):
        ...
        queryname, fullpath, fullcigar, pathranges, qranges= graphcigar(args.ref, args.query, args.input, args.addinsert)
