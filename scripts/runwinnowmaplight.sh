...
if [[ "$mode" == "masking" ]]; then
    ...
    repfile="${database}_repetitive_k17_H0.1.txt"
    [[ ! -f "$repfile" ]] && {
        winnowmap index -k 17 -H 0.1 "$database" -o "$repfile"
    }

    # Additional profile-specific options
    extra_opts="-k 17 -w 2 -m $size -p 0.000 --no-end-flt -W $repfile"

else

    # Parameters for masked profile
    extra_opts="-k 19 -w 10 -m 300 -p 0.05 "
fi
