...
static void usage() {
    std::cerr
        ...
        << "[-r ref.fasta] [-p pattern1,pattern2,...] [-t threads]\n"
        ...
        << "  -t, --threads   Threads for independent sequence finalization (default: 1).\n"
        << "                  K-mer orientation learning remains ordered and deterministic.\n";
}
...
int main(int argc, char** argv) {
        ...
        if ((arg == "-i" || arg == "--in") && i + 1 < argc) input_file = argv[++i];
            ...
            try {
                ...
                if (value.empty() || !std::all_of(
                        value.begin(), value.end(), [](unsigned char base) {
                            return std::isdigit(base) != 0;
                        })) {
                ...
                thread_count = static_cast<std::size_t>(parsed);
    ...
    // Apply flips + fix strand markers independently. Orientation learning
    // above intentionally remains ordered because each record contributes
    // evidence used by later records.
    const auto finalize_entry = [&](FastaEntry& e) {
        int sign = 0;
    ...
    const std::size_t workers = std::min(
        thread_count, std::max<std::size_t>(1, entries.size()));
    ...
    const auto worker = [&]() {
        while (!stop.load(std::memory_order_acquire)) {
            const std::size_t index = next_entry.fetch_add(
                1, std::memory_order_relaxed);
            if (index >= entries.size()) return;
            try {
                finalize_entry(entries[index]);
            } catch (...) {
    ...
    workers_running.reserve(workers);
    for (std::size_t index = 0; index < workers; ++index) {
        workers_running.emplace_back(worker);
    }
    for (std::thread& thread : workers_running) thread.join();
